...
from win32process import GetModuleFileNameEx, GetWindowThreadProcessId  # 使用pid获取可执行文件位置
from win32api import OpenProcess
from win32con import PROCESS_QUERY_INFORMATION, PROCESS_QUERY_LIMITED_INFORMATION
...
class Defend:
    ...
    def __init__(self, pid: int, file: str):
        print("\nWelcome to ues the Defend(Version = {}) by 爱和牛奶的涛哥\n".format(VERSION))
        ...
        if self.config.get("outlog", False):
            ...
            logging.write_log("In defend.py ({}) INFO: 启动成功 (file= {})".format(pid_self, p_name))
    ...
    def restart(self, filename):
        if filename is not None:
            ...
            if exists(filename_without_argv) and not isdir(filename_without_argv):
                ...
                if self.config.get("outlog", False):
                    logging.write_log(
                        "In defend.py ({}) WARNING: 尝试打开 \"{}\" 时发现 \"{}\" 不存在或是一个文件夹无法打开".format(
            ...
            if self.__pid is None or self.__pid < 0:
                if self.config.get("outlog", False):
                    if self.__find_filename <= 0:
                        logging.write_log("In defend.py ({}) INFO: 尝试获取 {} 的 pid".format(pid_self, p_name))
                    else:
                        logging.write_log(
                            "In defend.py ({}) WARNING: 获取 {} 的 pid 失败, 已失败 {}/{} (<=0 未无限制)".format(
                                pid_self,
                                p_name,
                ...
                if self.__pid is None:
                    ...
                    if self.config.get("outlog", False):
                        logging.write_log(
                            "In defend.py ({}) INFO: 获取 {} 的 pid 成功".format(pid_self, p_name))
                    self.__pid = self.get_pid_by_filename(p_name)
                    self.__find_filename = 0
            elif self.__pid == 0:
                ...
                if not pid_exists(self.pid):
                    logging.write_log(
                        "In defend.py ({}) INFO: {} ({}) 被关闭, 正在尝试打开".format(pid_self, p_name, self.pid))
                    ...
                    if func is not None:
                        ...
                        if return_code is None or return_code[0] is False:
                            self.restarted = True
                            logging.write_log(
                                "In defend.py ({}) WARNING: {} 打开失败, 已尝试 {}/{} (<=0 未无限制)".format(
                                    pid_self,
                                    p_name,
                                    self.range,
                                    max_range + 1))
                            ...
                            self.range = 1
                            logging.write_log(
                                "In defend.py ({}) INFO: {} 打开成功".format(pid_self, p_name))
...
if __name__ == "__main__":
    ...
    if __pid is None:
        logging.write_log("In defend.py ({}) ERROR: error_code: {} error_info: {} 已退出".format(
            pid_self,
            __error_code,
            __error_info))
    ...
    logging.write_log(
        "In defend.py ({}) ERROR: error_code: {} error_info: {} 已退出并重启此程序".format(pid_self, *res))
